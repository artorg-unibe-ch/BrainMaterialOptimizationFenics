...
def LoadCaseDefinition(LoadCase, FinalRelativeStretch, RelativeStepSize, Dimensions):
...
def BCsDefinition(Dimensions, Mesh, V, u_0, u_1):
...
def SolveProblem(LoadCase, FinalRelativeStretch, RelativeStepSize, Dimensions, NumberElements, Mesh, V, u, du, v, Ic, J, F, Psi, Plot = False):
    
    if LoadCase == 'Compression':
                
        # Load case
        [u_0, u_1, InitialState, Direction, Normal, NumberSteps, DeltaStretch] = LoadCaseDefinition(LoadCase, FinalRelativeStretch, RelativeStepSize, Dimensions)
        ...
        # Load case
        [u_0, u_1, InitialState, Direction, Normal, NumberSteps, DeltaStretch] = LoadCaseDefinition(LoadCase, FinalRelativeStretch*2, RelativeStepSize*2, Dimensions)

    # Boundary conditions
    [BoundaryConditions, ds] = BCsDefinition(Dimensions, Mesh, V, u_0, u_1)
...
def CostFunction(Parameters, Dimensions, NumberElements, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Plot = False):
    ...
    if 'Compression' in LoadCases:
        ...
        # Solve
        DataFrame = SolveProblem(LoadCase, FinalRelativeStretch, RelativeStepSize, Dimensions, NumberElements, Mesh, V, u, du, v, Ic, J, F, Psi)
...
def ParametersOptimization(ConstitutiveModel, NumberElements, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Dimensions=[5,5,5], Plot = False):
    ...
    if ConstitutiveModel == 'Ogden':
        ...
        Bounds = [(0.4, 0.495), (1E-3, 2)]
        
        FileName.write('Nu Mu Lambda TotalCost\n')

    FileName.close()    
    
    ResultsOptimization = minimize(CostFunction, InitialGuess, args = (Dimensions, NumberElements, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize), method = 'L-BFGS-B', bounds = Bounds)
