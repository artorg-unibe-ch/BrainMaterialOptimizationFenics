...
def CompressibleNeoHookean(Lambda, Mu, Ic, J):
...
def Interpolation(Tissue, LoadCase, DataFrame, FinalRelativeStretch, RelativeStepSize, Plot = False):
    
    # Experimental Data
    FolderPath = os.path.join('../ExperimentalData/')
        
    if Tissue == 'Cortex':
        FilePath = os.path.join(FolderPath, 'C_' + LoadCase + '_ExpDat.csv')
    if Tissue == 'Basal Ganglia':
        FilePath = os.path.join(FolderPath, 'BG_' + LoadCase + '_ExpDat.csv')
    if Tissue == 'Corona Radiata':
        FilePath = os.path.join(FolderPath, 'CR_' + LoadCase + '_ExpDat.csv')
    if Tissue == 'Corpus Callosum':
        FilePath = os.path.join(FolderPath, 'CC_' + LoadCase + '_ExpDat.csv')
...
def CostFunction(Parameters, Tissue, ConstitutiveModel, BCsType, Dimensions, NumberElements, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Plot = False):
    ...
    Output_Path = os.path.join('Results', Tissue, ConstitutiveModel, BCsType)
    FileName = open(os.path.join(Output_Path, str(NumberElements) + 'Elements.txt'), 'a+')
    
    if len(Parameters) == 3:
        FileName.write('%.3f %.3f %.3f' % (Lambda, Mu, Alpha))
    elif len(Parameters) == 2:
        FileName.write('%.3f %.3f %.3f' % (Lambda, Mu))
    ...
    if 'Compression' in LoadCases:
        ...
        if len(Parameters) == 3:
            Psi = CompressibleOgden(Lambda, Mu, Alpha, C, Ic, J)

        elif len(Parameters) == 2:
            Psi = CompressibleNeoHookean(Lambda, Mu, Ic, J)
        ...
        # Interpolation
        [XInterp, InterpExpData, InterpSimPred] = Interpolation(Tissue, LoadCase, DataFrame, FinalRelativeStretch, RelativeStepSize, Plot)
    ...
    FileName = open(os.path.join(Output_Path, str(NumberElements) + 'Elements.txt'), 'a+')
    FileName.write(' %.3f %.3f %.3f %.3f\n' % (np.sum(CompressionDelta2), np.sum(TensionDelta2), np.sum(SimpleShearDelta2), np.sum(TotalCost)))
    FileName.close()    
...
def ParametersOptimization(Tissue, ConstitutiveModel, NumberElements, BCsType, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Dimensions=[5,5,5], Plot = False):
    ...
    if ConstitutiveModel == 'Ogden':
        ...
        InitialGuess = np.array([Lambda, Mu, Alpha])
        Bounds = [(1, 1E6), (1E-1, 2), (-100, -1E-3)]
        
        FileName.write('Lambda Mu Alpha CompressionNormMax TensionNormMax SimpleShearNormMax TotalCostNormMax\n')
        ...
        InitialGuess = np.array([Lambda, Mu])
        Bounds = [(1, 1E3), (0.5, 2)]
        
        FileName.write('Lambda Mu CompressionNormMax TensionNormMax SimpleShearNormMax TotalCostNormMax\n')

    FileName.close()      
    
    ResultsOptimization = minimize(CostFunction, InitialGuess, args = (Tissue, ConstitutiveModel, BCsType, Dimensions, NumberElements, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize), method = 'L-BFGS-B', bounds = Bounds)
    
    if ConstitutiveModel == 'Ogden':
        [Lambda, Mu, Alpha] = ResultsOptimization.x
        print('Final Lambda = %1.3f' % (Lambda))
        print('Final Mu = %1.3f' % (Mu))
        print('Final Alpha = %1.3f' % (Alpha))
        
    elif ConstitutiveModel == 'Neo-Hookean':
        [Lambda, Mu] = ResultsOptimization.x
        print('Final Lambda = %1.3f' % (Lambda))
        print('Final Mu = %1.3f' % (Mu))
