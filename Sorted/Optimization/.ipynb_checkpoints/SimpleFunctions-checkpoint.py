...
def BCsDefinition(Dimensions, Mesh, V, u_0, u_1, LoadCase, BCsType=False):
    ...
    Domains_Facets = fe.MeshFunction('size_t', Mesh, 2)
...
def Interpolation(LoadCase, DataFrame, FinalRelativeStretch, RelativeStepSize, Plot = False):
    ...
    # Experimental Data
    FolderPath = os.path.join('/home/ubelix/artorg/ms20s284/repositories/material-model-optimization/FEniCS/ExperimentalData/')
...
def CostFunction(Parameters, ConstitutiveModel, BCsType, NumberElements, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Dimensions = [5,5,5], Plot = False):
    ...
    if 'Compression' in LoadCases:
        ...
        # Solve
        CompressionDataFrame = SolveProblem(LoadCase, ConstitutiveModel, BCsType, FinalRelativeStretch, RelativeStepSize, Dimensions, NumberElements, Mesh, V, u, du, v, Ic, J, F, Psi)

        # Interpolation
        [XInterp, InterpExpData, InterpSimPred] = Interpolation(LoadCase, CompressionDataFrame, FinalRelativeStretch, RelativeStepSize, Plot)
        ...
        for X in XInterp:
            CompressionStep.append(((InterpExpData(X)-InterpSimPred(X))/ InterpExpData(XInterp[i]))**2)
            CompressionNoNorm.append(((InterpExpData(X)-InterpSimPred(X)))**2)
            CompressionNormMax.append(((InterpExpData(X)-InterpSimPred(X))/ InterpExpData(XInterp[-1]))**2)
            i += 1
    ...
    if 'Tension' in LoadCases:
        ...
        # Solve
        TensileDataFrame = SolveProblem(LoadCase, ConstitutiveModel, BCsType, FinalRelativeStretch, RelativeStepSize, Dimensions, NumberElements, Mesh, V, u, du, v, Ic, J, F, Psi)

        # Interpolation
        [XInterp, InterpExpData, InterpSimPred] = Interpolation(LoadCase, TensileDataFrame, FinalRelativeStretch, RelativeStepSize, Plot)
        ...
        for X in XInterp:
            TensionStep.append(((InterpExpData(X)-InterpSimPred(X))/ InterpExpData(XInterp[i]))**2)
            TensionNoNorm.append(((InterpExpData(X)-InterpSimPred(X)))**2)
            TensionNormMax.append(((InterpExpData(X)-InterpSimPred(X))/ InterpExpData(XInterp[-1]))**2)
            i += 1
    ...
    if 'SimpleShear' in LoadCases:
        ...
        # Solve
        SimpleShearDataFrame = SolveProblem(LoadCase, ConstitutiveModel, BCsType, FinalRelativeStretch, RelativeStepSize, Dimensions, NumberElements, Mesh, V, u, du, v, Ic, J, F, Psi)

        # Interpolation
        [XInterp, InterpExpData, InterpSimPred] = Interpolation(LoadCase, SimpleShearDataFrame, FinalRelativeStretch, RelativeStepSize, Plot)
        ...
        for X in XInterp:
            SimpleShearStep.append(((InterpExpData(X)-InterpSimPred(X))/ InterpExpData(XInterp[i]))**2)
            SimpleShearNoNorm.append(((InterpExpData(X)-InterpSimPred(X)))**2)
            SimpleShearNormMax.append(((InterpExpData(X)-InterpSimPred(X))/ InterpExpData(XInterp[-1]))**2)
            i += 1

            
    # Compute total cost
    if 'Compression' in LoadCases and 'Tension' in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostStep = np.sum(CompressionStep)  * RelativeWeights[0] + np.sum(TensionStep) * RelativeWeights[1] + np.sum(SimpleShearStep) * RelativeWeights[2]
        TotalCostStep = TotalCostStep / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostStep = np.sum(CompressionStep)  * RelativeWeights[0] + np.sum(TensionStep) * RelativeWeights[1]
        TotalCostStep = TotalCostStep / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostStep = np.sum(CompressionStep)  * RelativeWeights[0] + np.sum(SimpleShearStep) * RelativeWeights[1]
        TotalCostStep = TotalCostStep / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostStep = np.sum(TensionStep)  * RelativeWeights[0] + np.sum(SimpleShearStep) * RelativeWeights[1]
        TotalCostStep = TotalCostStep / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostStep = np.sum(CompressionStep)  * RelativeWeights[0]
        TotalCostStep = TotalCostStep / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostStep = np.sum(TensionStep)  * RelativeWeights[0]
        TotalCostStep = TotalCostStep / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostStep = np.sum(SimpleShearStep) * RelativeWeights[0]
        TotalCostStep = TotalCostStep / np.sum(RelativeWeights)
        
    if 'Compression' in LoadCases and 'Tension' in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNoNorm = np.sum(CompressionNoNorm)  * RelativeWeights[0] + np.sum(TensionNoNorm) * RelativeWeights[1] + np.sum(SimpleShearNoNorm) * RelativeWeights[2]
        TotalCostNoNorm = TotalCostNoNorm / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostNoNorm = np.sum(CompressionNoNorm)  * RelativeWeights[0] + np.sum(TensionNoNorm) * RelativeWeights[1]
        TotalCostNoNorm = TotalCostNoNorm / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNoNorm = np.sum(CompressionNoNorm)  * RelativeWeights[0] + np.sum(SimpleShearNoNorm) * RelativeWeights[1]
        TotalCostNoNorm = TotalCostNoNorm / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNoNorm = np.sum(TensionNoNorm)  * RelativeWeights[0] + np.sum(SimpleShearNoNorm) * RelativeWeights[1]
        TotalCostNoNorm = TotalCostNoNorm / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostNoNorm = np.sum(CompressionNoNorm)  * RelativeWeights[0]
        TotalCostNoNorm = TotalCostNoNorm / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostNoNorm = np.sum(TensionNoNorm)  * RelativeWeights[0]
        TotalCostNoNorm = TotalCostNoNorm / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNoNorm = np.sum(SimpleShearNoNorm) * RelativeWeights[0]
        TotalCostNoNorm = TotalCostNoNorm / np.sum(RelativeWeights)
        
    if 'Compression' in LoadCases and 'Tension' in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNormMax = np.sum(CompressionNormMax)  * RelativeWeights[0] + np.sum(TensionNormMax) * RelativeWeights[1] + np.sum(SimpleShearNormMax) * RelativeWeights[2]
        TotalCostNormMax = TotalCostNormMax / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostNormMax = np.sum(CompressionNormMax)  * RelativeWeights[0] + np.sum(TensionNormMax) * RelativeWeights[1]
        TotalCostNormMax = TotalCostNormMax / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNormMax = np.sum(CompressionNormMax)  * RelativeWeights[0] + np.sum(SimpleShearNormMax) * RelativeWeights[1]
        TotalCostNormMax = TotalCostNormMax / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNormMax = np.sum(TensionNormMax)  * RelativeWeights[0] + np.sum(SimpleShearNormMax) * RelativeWeights[1]
        TotalCostNormMax = TotalCostNormMax / np.sum(RelativeWeights)
    elif 'Compression' in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostNormMax = np.sum(CompressionNormMax)  * RelativeWeights[0]
        TotalCostNormMax = TotalCostNormMax / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' in LoadCases and 'SimpleShear' not in LoadCases:
        TotalCostNormMax = np.sum(TensionNormMax)  * RelativeWeights[0]
        TotalCostNormMax = TotalCostNormMax / np.sum(RelativeWeights)
    elif 'Compression' not in LoadCases and 'Tension' not in LoadCases and 'SimpleShear' in LoadCases:
        TotalCostNormMax = np.sum(SimpleShearNormMax) * RelativeWeights[0]
        TotalCostNormMax = TotalCostNormMax / np.sum(RelativeWeights)
            
    print('Cost:', np.sum(TotalCostStep))
    
    #FileName = open(os.path.join(Output_Path, str(NumberElements) + 'Elements.txt'), 'a+')
    #FileName.write(' %.3f\n' % (TotalCostStep))    
    #FileName.close()    

    return np.sum(TotalCostStep), np.sum(CompressionStep), np.sum(TensionStep), np.sum(SimpleShearStep), np.sum(TotalCostNoNorm), np.sum(CompressionNoNorm), np.sum(TensionNoNorm), np.sum(SimpleShearNoNorm), np.sum(TotalCostNormMax), np.sum(CompressionNormMax), np.sum(TensionNormMax), np.sum(SimpleShearNormMax), CompressionDataFrame, TensileDataFrame, SimpleShearDataFrame
...
def ParametersOptimization(ConstitutiveModel, NumberElements, BCsType, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Dimensions=[5,5,5], Plot = False):
    ...
    if ConstitutiveModel == 'Ogden':
        ...
        FileName.write('Nu Mu Alpha TotalCostStep\n')
        ...
        FileName.write('Nu Mu Lambda TotalCostStep\n')
    ...
    if Plot == True :
        ...
        ax.cla()
        ax.plot(df.TotalCostStep,  color = 'g', linestyle = '--', marker = 'o', markersize = 8, fillstyle='none')
        ax.set_xlabel('Iteration number(-)')
        ax.set_ylabel('Cost (-)')
