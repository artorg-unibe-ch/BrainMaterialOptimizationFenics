...
def OgdenCostFunction(Parameters, Nu, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Dimensions, Plot = False):
    ...
    FileName = open('OptimizationResults.txt', 'a+')
...
def OgdenOptimization(Nu, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Dimensions, Plot = False):
    ...
    ResOpt = minimize(OgdenCostFunction, InitialGuess, args = (Nu, LoadCases, RelativeWeights, FinalRelativeStretch, RelativeStepSize, Dimensions), method = 'Nelder-Mead', options={'xatol':1E-1})
    
    [Mu, Alpha] = ResOpt.x
    print('Final Mu = ', Mu)
    print('Final Alpha =', Alpha)
    
    df = pd.read_csv('OptimizationResults.txt', sep=' ', decimal='.')
